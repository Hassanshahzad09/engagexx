...
export default function LoginSignup({ onLogin, onBack }) {
  ...
  let [signInBtn,setSignBtn] = useState(false)
  let[signInUser,setSignIn] = useState({
    "fullName":"John Doe",
  ...
  let [loginInUser,setLogin] = useState({
    "userEmail":"",
    "userPassword" : ""
  })
  ...
  try {
    ...
    setTimeout(()=>{
      ...
      if (response.status === 200) {
        ...
        const user = {'userName':data.firstName,"email":data.email,"userId":data.userId}
        navigate('/buyer-dashboard',{
          state:{userData:user}
        });
       // onLogin(userType); // or whatever
  ...
  if (signInUser.userName !== "") {
    ...
    const createAccount = async () => {
      try {
        ...
        // ✅ Important: stop loading after API completes
        setLoading(false);
      }
    }
...
setSignIn(prev=>({
  ...prev,"userName":(firstName + lastName).toLowerCase() + randomDigits,
  "firstName":firstName,
  "lastName":lastName
}))
...
console.log(signInUser)
  //firstName,lastName,userEmail,userName,userPassword(confirmedPassword),userType
          ...
          <CardContent className="p-4 sm:p-6">
            <Tabs value={isLogin ? 'login' : 'signup'} onValueChange={(v) => setIsLogin(v === 'login')}>
              <TabsList className="w-full mb-6">
                <TabsTrigger value="login" className="flex-1 text-sm sm:text-base">
                  Login
                </TabsTrigger>
                <TabsTrigger value="signup" className="flex-1 text-sm sm:text-base">
                  Sign Up
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="login" className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-10"
                      value = {loginInUser.userEmail}
                      onChange={(e)=>{setLogin(prev=>({...prev,"userEmail":e.target.value}))}}
                    />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder="••••••••"
                      className="pl-10"
                     value = {loginInUser.userPassword}
                     onChange={(e)=>{setLogin(prev=>({...prev,"userPassword":e.target.value}))}}
                    />
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 text-sm">
                  ...
                  <a href="#" className="text-green-600 hover:text-green-700 text-xs sm:text-sm">
                    Forgot password?
                  </a>
                </div>
